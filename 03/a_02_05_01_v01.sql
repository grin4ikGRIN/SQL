...
select * from produkt where euro_preis > 20 and euro_preis < 50 order by produkt_name, euro_preis asc;
select kunde_id, datum from abrechnung;
select * from kunde order by kunde_id asc limit 2,4;
select distinct kunde_id from abrechnung order by kunde_id;
